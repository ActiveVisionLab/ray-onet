...
def sample_plane_feature(xy, c):
    ...
    Args:
        xy (tensor): sample point (x, y)
        c (tensor): feature map
    '''
    xy = xy[:, :, None].float()
    xy = 2.0 * xy - 1.0  # normalize to (-1, 1)
    c = F.grid_sample(c, xy, padding_mode='border', align_corners=True, mode='bilinear').squeeze(-1)
    # c = F.grid_sample(c, vgrid, padding_mode='border', mode='bilinear').squeeze(-1) # for pytorch < 1.4
    return c

def origin_to_world(n_points,world_mat):
    ...
    device = world_mat.device

    p = torch.zeros(batch_size, n_points, 3).to(device)
    p_world = transform_points_back(p, world_mat)
    return p_world