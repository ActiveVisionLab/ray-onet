...
for c in ${CLASSES[@]}; do
  ...
  camera_path_c=$ONET_DATA_PATH/$c
  ...
  echo "Generate ground truth occupancies for rays from each view"
  python sample_mesh.py $build_path_c/2_watertight \
      --n_proc $NPROC \
      --camera_folder $camera_path_c \
      --points_folder $build_path_c/ray_occ \
      --packbits
done